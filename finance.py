...
if data:
    ...
    df = handle_data(data)
    print(df)
    print(df.drop(columns=['Date', 'Label']))
    print(trading_bot)
    print('columns', len(df.columns))
    print('features', trading_bot.max_features_)

    for i in range(len(df) - 1):
        pred = trading_bot.predict(df.drop(columns=['Date', 'Label']).iloc[i:i+1])
        # if pred != actual:
        print(f'Price: {df["Close"].iloc[i]:.2f}, Prediction: {pred} --- Next Price: {df["Close"].iloc[i+1]:.2f}')
...
# start_date = datetime(2007,1,9)
# end_date = datetime(2023,3,9) 
# broker = Alpaca(ALPACA_CREDS) 
# strategy = MLTrader(name='mlstrat', broker=broker, 
#                     parameters={"ticker":"SPY", 
#                                 "cash_at_risk":.5})
# strategy.backtest(
#     YahooDataBacktesting, 
#     start_date, 
#     end_date, 
#     parameters={"ticker":ticker, "cash_at_risk":.5}
# )
# # trader = Trader()
